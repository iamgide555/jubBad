...
| 33 | *Implemented 2026-09-07.* **"Best partner" did not match user expectations.** The documented rule ranks wins, then more games played, so it chooses 2 wins from 9 over 2 from 2. `server/src/groups/groups.service.ts` | Took the labelling option, not a metric change: the backlog chose this rule deliberately and a win rate over two matches is noise. The API field is now `mostWinsWith`, the profile heading reads "ชนะด้วยกันมากที่สุด" / "Most wins together", and the contradictory inline example — which claimed the opposite of what the code does — is replaced by the real rule and the reason for it. A server test pins the tie-break so the name and the arithmetic cannot drift apart again. |
