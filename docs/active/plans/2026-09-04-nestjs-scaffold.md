...
- **A stale incremental `tsconfig.build.tsbuildinfo` cache from before the `rootDir` change can transiently pollute the repo root.** Confirmed during execution: after widening `rootDir` in Task 2, the very next `nest build` (reusing a `.tsbuildinfo` written earlier under `rootDir: "./src"`) emitted `pairing.js`/`.d.ts`/`.js.map` at the **repo root** (sibling to `server/`, `pairing.ts`) instead of inside `server/dist/`. Deleting `server/dist` (which also removes the nested `.tsbuildinfo`, gitignored via the root `*.tsbuildinfo` pattern) and rebuilding came out clean — confirmed twice. If this project has ever been built with the old `rootDir`, delete `server/dist` before Task 2 Step 4's rebuild, and check `git status` at the repo root (not just inside `server/`) after any `nest build` here before committing.
...
Expected: completes with no output (NestJS's build is silent on success) and produces `dist/main.js` (this default flat layout is only correct *before* Task 2 widens `rootDir` — it moves to `dist/server/src/main.js` in Task 2).
