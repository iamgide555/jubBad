...
Expected: PASS. Note: this test needs `DATABASE_URL` in `process.env`, which `server/vitest.config.ts` already loads via `import 'dotenv/config'` (done in the Prisma schema plan) — no change needed there.
